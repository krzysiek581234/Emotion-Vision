...
testpath = 'D:/Studia_Krzysiek/Semestr4/Projekty/Projekt_Sztuczna/Emotion-Vision/dataset/testdataTL'
seed = 1444
...
def main():
    ...
    logdir = os.path.join(log_dir, current_time + '_' + socket.gethostname())
    ...
    if TRAIN:
        train(model, train_loader,val_loader, optimizer, criterion, num_epochs, writer)
    else:
        # Load for tests: resnet-transfer-30.pth
        #state_dict = torch.load('../savedModels/TL.pth', map_location=torch.device('cuda'))
        state_dict = torch.load('resnet-transfer-30.pth', map_location=torch.device('cuda'))
